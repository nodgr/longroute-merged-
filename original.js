...
function routecut(route){//ルート中のどこかの地点から探索をやり直す
    var randr = Math.floor(Math.random()*(route.length));//進んできた経路の任意の一点を選択
    var tmplength = route.length;//ルート改変前の長さ
    pos=route[randr];//現在位置をやり直し始める地点にする
    for(var i=randr;i<route.length;i++){
        board[route[i]] = 0;//やり直し地点以降のルートをなかったことにする
        document.getElementById('s'+route[i]).innerHTML='';//矢印を削除
    }
    route.splice(randr,(tmplength));//以降のルートを削除
    for( var i=0;i<=route.length;i++){//消されてないルートを探索中にする
        board[route[i]] = 1;
    }
    board[pos] = 1;//現在位置
    board[88] = 0;//ゴールも未探索にする
    idtoBoard('s');//情報をhtmlに反映
}